    ...
    <footer className={"bg-[#3c162e]"}>
      <Container size="xl" py="xl">
        ...
        <SimpleGrid spacing="lg" mt="xl" cols={{ base: 1, md: 4 }}>
          <div>
            <Title order={5} c="#FFC7C7">
              Quick Links
            </Title>
            {quickLinks.map((link, index) => (
              <Text key={index} size="sm" c={"white"} py={1}>
                {link}
              </Text>
            ))}
          </div>
        ...
        <div className="mt-12 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Image src={logo} h={50} w="auto" fit="contain" />
          </div>
        ...
        >
          <Text c={"white"} size={"xs"}>
            Operated by{" "}
            <Link to={"https://www.tradenet.com.mv/"}>Tradenet</Link>
          </Text>
          <Text c={"white"} size={"xs"}>
            © 2024, Government of Maldives. All Rights Reserved.
          </Text>
        </Flex>
      </Container>
    </footer>
